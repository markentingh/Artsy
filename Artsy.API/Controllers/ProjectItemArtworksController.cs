...
namespace Artsy.API.Controllers
{
    ...
    public partial class ProjectsController
    {
        ...
        [HttpPost("update-item-opacity")]
        public async Task<IActionResult> UpdateItemOpacity([FromBody] UpdateProjectItemOpacityRequest request)
        {
            var userId = GetUserId();
            if (userId == Guid.Empty)
                return Json(new ApiResponse { success = false, message = "Could not find user" });
            ...
            try
            {
                ...
                var artworkList = await _projectItemArtworkRepository.GetByItemIdAsync(request.ItemId);
                var artwork = artworkList.FirstOrDefault();
                var opacityJson = string.IsNullOrWhiteSpace(request.OpacityJson) ? null : request.OpacityJson;
