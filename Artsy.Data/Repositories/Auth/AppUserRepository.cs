...
namespace Artsy.Data.Repositories.Auth
{
    public class AppUserRepository : IAppUserRepository, IDisposable
    {
        ...
        {
            var validColumns = new HashSet<string> { "Email", "FullName", "Created", "AR.Name", "LastLogin" };
            ...
            {
                "Email" => $"SU.\"Email\" {direction}",
                "FullName" => $"SU.\"FullName\" {direction}",
                "Created" => $"SU.\"Created\" {direction}",
                "AR.Name" => $"AR.\"Name\" {direction}",
                "LastLogin" => $"\"LastLogin\" {direction}",
                _ => "SU.\"Email\" ASC"
            };
        }

        public async Task<(IList<FilteredUserResult> items, int totalCount)> GetAllFiltered(string fullName, int role, string sort, int page = 1, int pageSize = 10)
        {
            ...
            {
                countQuery += @" AND (SU.""FullName"" LIKE @FullName OR SU.""Email"" LIKE @FullName)";
            }
            ...
            if (!string.IsNullOrEmpty(fullName))
            {
                query += @" AND (SU.""FullName"" LIKE @FullName OR SU.""Email"" LIKE @FullName)";
            }
        ...
        public async Task<AppUser> Add(AppUser user)
        {
            ...
            foreach (var role in (user.UserRoles ?? new List<AppUserRole>()).Where(x => x.AppRoleId != 0))
            {
        ...
        public async Task<bool> UpdateLock(Guid userId, bool lockUser)
        {
            ...
            var affected = await _dbConnection.ExecuteAsync(query, new { Status = statusValue, UserId = userId });
            return affected > 0;
        }

        public async Task<AppUser?> FindByOAuthStateAsync(string state)
        {
            string query = @$"SELECT * FROM {_tableName} WHERE ""OAuthState"" = @State";
            return await _dbConnection.QueryFirstOrDefaultAsync<AppUser>(query, new { State = state });
        }

        public async Task<AppUser?> FindByOAuthStatePrefixAsync(string prefix)
        {
            string query = @$"SELECT * FROM {_tableName} WHERE ""OAuthState"" LIKE @Prefix || '%'";
            return await _dbConnection.QueryFirstOrDefaultAsync<AppUser>(query, new { Prefix = prefix });
        }

        public void UpdateOAuthState(AppUser user)
        {
            string query = @$"UPDATE {_tableName} SET ""OAuthState"" = @OAuthState WHERE ""Id"" = @Id";
            _dbConnection.Execute(query, new { user.Id, user.OAuthState });
        }

        public void UpdatePrintifyTokens(AppUser user)
        {
            string query = @$"UPDATE {_tableName} SET
                ""PrintifyAccessToken"" = @PrintifyAccessToken,
                ""PrintifyRefreshToken"" = @PrintifyRefreshToken,
                ""PrintifyTokensExpireAtUtc"" = @PrintifyTokensExpireAtUtc,
            ...
            {
                user.Id,
                user.PrintifyAccessToken,
                user.PrintifyRefreshToken,
                user.PrintifyTokensExpireAtUtc,
                user.PrintifyShopId,
                user.OAuthState
            });
        }

        public void UpdateTelegramConnection(AppUser user)
        {
            string query = @$"UPDATE {_tableName} SET
            ...
            _dbConnection.Execute(query, new
            {
                ...
                user.TelegramUserId,
                user.TelegramChatId,
                user.TelegramConnectionToken
            });
        }

        public async Task<AppUser?> FindByTelegramConnectionTokenAsync(string token)
        {
            string query = @$"SELECT * FROM {_tableName} WHERE ""TelegramConnectionToken"" = @Token";
            return await _dbConnection.QueryFirstOrDefaultAsync<AppUser>(query, new { Token = token });
        }

        public async Task<AppUser?> FindByTelegramUserIdAsync(string telegramUserId)
        {
            string query = @$"SELECT * FROM {_tableName} WHERE ""TelegramUserId"" = @TelegramUserId";
            return await _dbConnection.QueryFirstOrDefaultAsync<AppUser>(query, new { TelegramUserId = telegramUserId });
        }

        public void UpdateMetaTokens(AppUser user)
        {
            string query = @$"UPDATE {_tableName} SET
